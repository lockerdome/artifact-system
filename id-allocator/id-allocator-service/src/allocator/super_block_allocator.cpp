...
SuperBlockAllocator::SuperBlockAllocator(BlockStore& store, PartitionConfig config)
    : store_{store}, config_{std::move(config)}, bucket_selector_{store_, config_.partition_id, config_.num_buckets} {
}

std::expected<std::pair<uint64_t, uint64_t>, StoreError> SuperBlockAllocator::allocate_block() {
...
std::expected<SuperBlock, StoreError> SuperBlockAllocator::fetch_super_block_from(uint32_t bucket_index) {
  ...
  auto alloc_result = store_.allocate(config_.partition_id, bucket_index, config_.super_block_size, config_.max_bucket_value());
  ...
  uint64_t range_start = compose_id(bucket_index, config_.bucket_size_bits, previous_count);
  uint64_t range_end = compose_id(bucket_index, config_.bucket_size_bits, previous_count + config_.super_block_size);
...
void SuperBlockAllocator::maybe_start_prefetch() {
  ...
  bool below_threshold = active_.remaining() <= config_.super_block_size / 2;
  ...
  if (below_threshold && background_.is_empty()) {
    ...
    pending_fetch_ = std::async(std::launch::async, [this, bucket_index] { return fetch_super_block_from(bucket_index); });
  }
}
