...
std::expected<uint64_t, StoreError> MemoryStore::allocate(
...
std::expected<void, StoreError> MemoryStore::save_partition(
...
MemoryStore::get_partition(std::string_view partition_id) {
...
std::expected<void, StoreError> MemoryStore::delete_partition(
    std::string_view partition_id) {
