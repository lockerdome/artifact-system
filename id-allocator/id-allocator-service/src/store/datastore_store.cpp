...
DatastoreStore::DatastoreStore(std::string project_id, std::shared_ptr<grpc::Channel> channel)
    : project_id_(std::move(project_id)), stub_(ds::Datastore::NewStub(std::move(channel))) {
}
...
google::datastore::v1::Key DatastoreStore::make_key(std::string_view kind, std::string_view name) const {
  ds::Key key;
...
google::datastore::v1::Entity DatastoreStore::make_bucket_entity(std::string_view partition_id, uint32_t bucket_index, uint64_t count) const {
  ds::Entity entity;
  *entity.mutable_key() = make_key(bucket_kind(partition_id), bucket_key_name(bucket_index));
...
google::datastore::v1::Entity DatastoreStore::make_partition_entity(const PartitionConfig& config) const {
  ds::Entity entity;
  *entity.mutable_key() = make_key(kPartitionKind, config.partition_id);
  ...
  bucket_size_bits.set_integer_value(static_cast<int64_t>(config.bucket_size_bits));
  props["bucket_size_bits"] = std::move(bucket_size_bits);

  ds::Value super_block_size;
  super_block_size.set_integer_value(static_cast<int64_t>(config.super_block_size));
...
PartitionConfig DatastoreStore::entity_to_partition(const ds::Entity& entity) {
  PartitionConfig config;

  // The partition_id is the key's name.
  if (entity.key().path_size() > 0) {
    config.partition_id = entity.key().path(entity.key().path_size() - 1).name();
  }

  const auto& props = entity.properties();
  if (auto it = props.find("num_buckets"); it != props.end()) {
    config.num_buckets = static_cast<uint32_t>(it->second.integer_value());
  }
  if (auto it = props.find("bucket_size_bits"); it != props.end()) {
    config.bucket_size_bits = static_cast<uint32_t>(it->second.integer_value());
  }
  if (auto it = props.find("super_block_size"); it != props.end()) {
    config.super_block_size = static_cast<uint32_t>(it->second.integer_value());
  }
  if (auto it = props.find("block_size"); it != props.end()) {
    config.block_size = static_cast<uint32_t>(it->second.integer_value());
  }
...
std::expected<uint64_t, StoreError> DatastoreStore::allocate(std::string_view partition_id, uint32_t bucket_index, uint64_t increment, uint64_t max_value) {
  ...
  *lookup_req.add_keys() = make_key(bucket_kind(partition_id), bucket_key_name(bucket_index));
  ...
  *mutation->mutable_upsert() = make_bucket_entity(partition_id, bucket_index, current + increment);

  ds::CommitResponse commit_resp;
  grpc::ClientContext commit_ctx;
  auto commit_status = stub_->Commit(&commit_ctx, commit_req, &commit_resp);
  if (!commit_status.ok()) {
...
std::expected<std::unordered_map<uint32_t, uint64_t>, StoreError> DatastoreStore::get_bucket_counts(std::string_view partition_id,
  ...
  for (const auto& found : resp.found()) {
    ...
    if (entity.key().path_size() > 0) {
      const auto& key_name = entity.key().path(entity.key().path_size() - 1).name();
      // Parse bucket index from "bucket-{index}".
      if (key_name.starts_with("bucket-")) {
        uint32_t idx = 0;
        auto [ptr, ec] = std::from_chars(key_name.data() + 7, key_name.data() + key_name.size(), idx);
        if (ec == std::errc{}) {
...
std::expected<void, StoreError> DatastoreStore::save_partition(const PartitionConfig& config) {
  ...
  ds::BeginTransactionResponse begin_resp;
  auto begin_status = stub_->BeginTransaction(&begin_ctx, begin_req, &begin_resp);
  if (!begin_status.ok()) {
    return std::unexpected(map_grpc_error(begin_status));
  }
  const std::string& txn = begin_resp.transaction();
...
std::expected<std::optional<PartitionConfig>, StoreError> DatastoreStore::get_partition(std::string_view partition_id) {
  ...
  *req.add_keys() = make_key(kPartitionKind, partition_id);
  ...
  return std::optional<PartitionConfig>{entity_to_partition(resp.found(0).entity())};
}

// ─── delete_partition() ─────────────────────────────────────────────────────

std::expected<void, StoreError> DatastoreStore::delete_partition(std::string_view partition_id) {
...
std::expected<std::vector<PartitionConfig>, StoreError> DatastoreStore::list_partitions() {
  ...
  for (;;) {
    ...
    if (batch.more_results() == ds::QueryResultBatch::NOT_FINISHED) {
