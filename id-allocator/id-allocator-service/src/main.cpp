...
std::unique_ptr<id_allocator::BlockStore> create_store(
    const id_allocator::ServerConfig& config) {
    ...
    if (config.store_type == "datastore") {
        if (config.gcp_project_id.empty()) {
            std::println(stderr,
                "ID_ALLOCATOR_GCP_PROJECT_ID is required for datastore store");
            std::exit(1);
        }

        std::string endpoint = config.datastore_endpoint.empty()
            ? "datastore.googleapis.com:443"
            : config.datastore_endpoint;

        std::println("Using Datastore store (project={}, endpoint={})",
                     config.gcp_project_id, endpoint);

        auto channel = grpc::CreateChannel(
            endpoint, grpc::GoogleDefaultCredentials());

        return std::make_unique<id_allocator::DatastoreStore>(
            config.gcp_project_id, std::move(channel));
    }
