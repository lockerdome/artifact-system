...
Server::Server(ServerConfig config, std::unique_ptr<BlockStore> store)
    : config_{std::move(config)}, store_{std::move(store)}, partition_manager_{*store_}, allocator_service_{partition_manager_},
...
void Server::run() {
  ...
  if (!grpc_server_) {
    std::println(stderr, "Failed to start gRPC server on {}", config_.listen_address);
    std::abort();
  }
