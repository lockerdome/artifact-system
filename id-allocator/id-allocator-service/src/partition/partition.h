...
struct PartitionConfig {
  ...
  [[nodiscard]] std::expected<void, std::string> validate() const {
    ...
    if (super_block_size % block_size != 0) {
      return std::unexpected("super_block_size must be an exact multiple of block_size");
    }
    ...
    if (num_buckets > max_buckets) {
      return std::unexpected("num_buckets exceeds the maximum representable with "
