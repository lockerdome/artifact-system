...
PartitionManager::PartitionManager(BlockStore& store) : store_(store) {
}
...
std::expected<void, StoreError> PartitionManager::initialize() {
  ...
  for (auto& config : *result) {
    ...
    partitions_.emplace(std::move(id), PartitionEntry{std::move(config), std::move(allocator)});
  }

  return {};
}

std::expected<void, StoreError> PartitionManager::create_partition(PartitionConfig config) {
...
std::expected<PartitionConfig, StoreError> PartitionManager::get_partition(std::string_view partition_id) {
...
std::expected<void, StoreError> PartitionManager::delete_partition(std::string_view partition_id) {
...
std::expected<std::pair<uint64_t, uint64_t>, StoreError> PartitionManager::allocate_block(std::string_view partition_id) {
