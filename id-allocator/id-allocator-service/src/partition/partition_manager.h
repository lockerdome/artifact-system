...
public:
  ...
  std::expected<PartitionConfig, StoreError> get_partition(std::string_view partition_id);

  /// Delete a partition. Removes from store and tears down allocator.
  /// Idempotent — returns OK even if partition doesn't exist.
  std::expected<void, StoreError> delete_partition(std::string_view partition_id);
  ...
  std::expected<std::pair<uint64_t, uint64_t>, StoreError> allocate_block(std::string_view partition_id);
