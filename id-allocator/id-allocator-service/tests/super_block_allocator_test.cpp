...
PartitionConfig make_config(uint32_t super_block_size = 16, uint32_t block_size = 4) {
...
TEST(SuperBlockAllocatorTest, BasicAllocationReturnsNonOverlappingBlocks) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    EXPECT_EQ(end - start, config.block_size) << "block " << i << ": range_start=" << start << " range_end=" << end;
  ...
  for (size_t i = 0; i < ranges.size(); ++i) {
    for (size_t j = i + 1; j < ranges.size(); ++j) {
      auto [s1, e1] = ranges[i];
      auto [s2, e2] = ranges[j];
      EXPECT_TRUE(e1 <= s2 || e2 <= s1) << "ranges overlap: [" << s1 << ", " << e1 << ") and [" << s2 << ", " << e2 << ")";
    }
  }
}
...
TEST(SuperBlockAllocatorTest, ReturnedIdsAreValidComposedIds) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    EXPECT_EQ(compose_id(bucket, config.bucket_size_bits, counter), start) << "block " << i;
  }
}
...
TEST(SuperBlockAllocatorTest, AllReturnedRangesAreNonOverlapping) {
  ...
  for (int i = 0; i < kTotalBlocks; ++i) {
    ...
    for (uint64_t id = start; id < end; ++id) {
      ...
      EXPECT_TRUE(inserted) << "Duplicate ID " << id << " in block " << i;
    }
  }

  EXPECT_EQ(all_ids.size(), static_cast<size_t>(kTotalBlocks * config.block_size));
}
...
TEST(SuperBlockAllocatorTest, ConcurrentAllocationsProduceValidRanges) {
  ...
  std::vector<std::vector<std::pair<uint64_t, uint64_t>>> per_thread_ranges(kNumThreads);
  ...
  EXPECT_EQ(all_ids.size(), static_cast<size_t>(kNumThreads * kBlocksPerThread * config.block_size));
}
