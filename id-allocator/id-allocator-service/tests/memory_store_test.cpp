...
TEST(MemoryStoreTest, ConcurrentAllocationsProduceNonOverlappingRanges) {
  ...
  constexpr uint64_t kMaxValue = static_cast<uint64_t>(kNumThreads) * kIterationsPerThread;
  ...
  auto worker = [&](int thread_idx) {
    for (int i = 0; i < kIterationsPerThread; ++i) {
      auto result = store.allocate("test-partition", 0, kIncrement, kMaxValue);
      if (!result.has_value()) {
  ...
  EXPECT_EQ(all_values.size(), static_cast<size_t>(kNumThreads * kIterationsPerThread));

  // Final counter should equal total increments.
  auto final_result = store.get_bucket_counts("test-partition", std::vector<uint32_t>{0});
