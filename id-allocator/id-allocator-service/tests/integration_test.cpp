...
protected:
  void SetUp() override {
    ...
    admin_service_ = std::make_unique<IdAllocatorAdminServiceImpl>(*partition_manager_);
    allocator_service_ = std::make_unique<IdAllocatorServiceImpl>(*partition_manager_);
  ...
  grpc::Status CreateTestPartition(const std::string& partition_id, uint32_t block_size = 2048, uint32_t super_block_size = 65536) {
...
TEST_F(IntegrationTest, AllocateBlockMultipleTimesNonOverlapping) {
  ...
  for (int i = 0; i < kAllocations; ++i) {
    ...
    grpc::ClientContext ctx;
    auto status = allocator_stub_->AllocateBlock(&ctx, req, &resp);
    ASSERT_TRUE(status.ok()) << "allocation " << i << ": " << status.error_message();

    for (uint64_t id = resp.range_start(); id < resp.range_end(); ++id) {
      auto [_, inserted] = all_ids.insert(id);
      EXPECT_TRUE(inserted) << "Duplicate ID " << id << " in allocation " << i;
    }
  }
