...
absl::Status MakeFetchIndexError(absl::StatusCode code,
                                 const std::string& message,
                                 FetchIndexError::Category category,
...
absl::Status MakeSnapshotTxnError(const std::string& message,
                                  SnapshotTransactionError::Category category,
                                  const std::string& id) {
...
IndexServiceImpl::IndexServiceImpl(StorageInterface* storage,
                                   transaction::TransactionManager* txn_manager,
                                   registry::TypeRegistry* registry)
    : storage_(storage), txn_manager_(txn_manager), registry_(registry) {}

absl::StatusOr<std::string> IndexServiceImpl::ResolveReadRef(
  ...
  if (read_context.has_snapshot_id()) {
    ...
    if (!meta.ok()) {
      return MakeSnapshotTxnError(
          std::string(meta.status().message()),
          SnapshotTransactionError::SNAPSHOT_NOT_FOUND,
          read_context.snapshot_id());
    }
    return read_context.snapshot_id();
  }
  if (read_context.has_transaction_id()) {
    auto meta =
        txn_manager_->GetTransactionMetadata(read_context.transaction_id());
    if (!meta.ok()) {
      return MakeSnapshotTxnError(
          std::string(meta.status().message()),
          SnapshotTransactionError::TRANSACTION_NOT_FOUND,
          read_context.transaction_id());
    }
...
grpc::Status IndexServiceImpl::FetchIndex(grpc::ServerContext* /*context*/,
                                          const FetchIndexRequest* request,
                                          FetchIndexResponse* response) {
  ...
  if (!schema_info_or.ok()) {
    if (absl::IsNotFound(schema_info_or.status())) {
      return AbslToGrpcStatus(MakeFetchIndexError(
          absl::StatusCode::kNotFound,
          absl::StrCat("index not found for key_type: ", key_type),
  ...
  const google::protobuf::Descriptor* key_desc =
      artifact::BuildPoolAndFindMessage(schema_info.index_descriptor_set,
                                        schema_info.key_message_name, &pool);
  if (key_desc == nullptr) {
    return AbslToGrpcStatus(MakeFetchIndexError(
        absl::StatusCode::kInternal,
        absl::StrCat("failed to build key descriptor for key_type: ",
                      key_type),
  ...
  const google::protobuf::Message* key_prototype =
      factory.GetPrototype(key_desc);
  std::unique_ptr<google::protobuf::Message> key_msg(key_prototype->New());
  if (!key_msg->ParseFromString(request->key())) {
    return AbslToGrpcStatus(MakeFetchIndexError(
        absl::StatusCode::kInvalidArgument,
        absl::StrCat("failed to parse key bytes for key_type: ", key_type),
        FetchIndexError::KEY_PARSE_FAILURE, key_type));
  }
  ...
  for (int i = 0; i < key_desc->field_count(); ++i) {
    ...
    if (!refl->HasField(*key_msg, field)) {
      return AbslToGrpcStatus(MakeFetchIndexError(
          absl::StatusCode::kInvalidArgument,
          absl::StrCat("missing key field '", field->name(),
                        "' for key_type: ", key_type),
          FetchIndexError::INCOMPLETE_KEY, key_type));
    }
  }

  // Step 6: Encode the key fields to a deterministic binary representation.
  auto encoded_key_or = encoding::EncodeKey(*key_desc, *key_msg,
                                            schema_info.key_fields);
  if (!encoded_key_or.ok()) {
    return AbslToGrpcStatus(MakeFetchIndexError(
        absl::StatusCode::kInvalidArgument,
        absl::StrCat("failed to encode key for key_type: ", key_type, ": ",
                      encoded_key_or.status().message()),
        FetchIndexError::KEY_PARSE_FAILURE, key_type));
  }
  ...
  const std::string path =
      encoding::IndexPath(schema_info.index_definition_id, key_span);
  ...
  if (!data_or.ok()) {
    if (absl::IsNotFound(data_or.status())) {
      ...
      const google::protobuf::Descriptor* index_desc =
          pool.FindMessageTypeByName(schema_info.index_message_name);
      if (index_desc != nullptr) {
        const google::protobuf::Message* index_prototype =
            factory.GetPrototype(index_desc);
        std::unique_ptr<google::protobuf::Message> empty_msg(
            index_prototype->New());
