...
ArtifactLayerServer::ArtifactLayerServer(const ServerConfig& config)
...
absl::Status ArtifactLayerServer::Initialize() {
  ...
  impl_->id_allocator = std::make_unique<MockIdAllocator>(
      bootstrap::GenesisIds::kFirstUserAllocatableId);

  // 4. Create transaction manager
  impl_->txn_manager = std::make_unique<transaction::TransactionManager>(
      impl_->storage.get());
  ...
  impl_->artifact_store = std::make_unique<artifact::ArtifactStore>(
      impl_->storage.get(), impl_->txn_manager.get(),
      impl_->id_allocator.get(), store_options);

  // 6. Create type registry
  impl_->type_registry = std::make_unique<registry::TypeRegistry>(
      impl_->storage.get(), impl_->txn_manager.get(),
      impl_->id_allocator.get(),
      genesis_result->index_def_ids_by_key_type);

  // 7. Create service implementations
  impl_->snapshot_txn_service =
      std::make_unique<SnapshotTransactionServiceImpl>(
          impl_->txn_manager.get());
  impl_->artifact_service =
      std::make_unique<ArtifactServiceImpl>(impl_->artifact_store.get());
  impl_->index_service = std::make_unique<IndexServiceImpl>(
      impl_->storage.get(), impl_->txn_manager.get(),
      impl_->type_registry.get());
  impl_->type_registry_service =
      std::make_unique<TypeRegistryServiceImpl>(impl_->type_registry.get());
...
void ArtifactLayerServer::Start() {
  ...
  builder.AddListeningPort(impl_->config.listen_address,
  ...
  std::println("Artifact layer server listening on {}",
